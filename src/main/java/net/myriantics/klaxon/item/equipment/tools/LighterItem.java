...
public class LighterItem extends FlintAndSteelItem {
    ...
    public void releaseUsing(ItemStack stack, Level level, LivingEntity livingEntity, int timeCharged) {
        ...
        livingEntity.playSound(KlaxonSoundEvents.LIGHTER_CLOSE, 1.0F, level.getRandom().nextFloat() * 0.4F + 0.8F);
        livingEntity.level().gameEvent(livingEntity, GameEvent.ITEM_INTERACT_START, livingEntity.getEyePosition());
    }
    ...
    private void startUsing(Player player, InteractionHand hand) {
        ...
        player.playSound(KlaxonSoundEvents.LIGHTER_OPEN, 1.0F, player.level().getRandom().nextFloat() * 0.4F + 0.8F);
        player.level().gameEvent(player, GameEvent.ITEM_INTERACT_START, player.getEyePosition());
    }
    ...
    public SoundEvent getFirePlaceSound() {
        return KlaxonSoundEvents.LIGHTER_IGNITE_FIRE;
    }
}
