...
public abstract class GrappleWinchCableRenderer {
    ...
    public static void renderGrappleWinchCable(PlayerEntity player, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, Vec3d connectionPos, int blockLight, int endpointBlockLight) {
        matrices.push();
        double d = player.lerpYaw(tickDelta) * (float) (Math.PI / 180.0) + (Math.PI / 2);
        Vec3d vec3d2 = player.getLeashOffset(tickDelta);
        double e = Math.cos(d) * vec3d2.z + Math.sin(d) * vec3d2.x;
        double f = Math.sin(d) * vec3d2.z - Math.cos(d) * vec3d2.x;
        double g = MathHelper.lerp((double)tickDelta, player.prevX, player.getX()) + e;
        double h = MathHelper.lerp((double)tickDelta, player.prevY, player.getY()) + vec3d2.y;
        double i = MathHelper.lerp((double)tickDelta, player.prevZ, player.getZ()) + f;
        ...
        float o = l * n;
        float p = j * n;
        BlockPos blockPos = BlockPos.ofFloored(player.getCameraPosVec(tickDelta));
        BlockPos blockPos2 = BlockPos.ofFloored(connectionPos);
        int q = blockLight;
        int r = q;
        int s = player.getWorld().getLightLevel(LightType.SKY, blockPos);
        int t = player.getWorld().getLightLevel(LightType.SKY, blockPos2);
