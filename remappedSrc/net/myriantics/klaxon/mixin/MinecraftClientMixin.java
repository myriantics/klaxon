...
public abstract class MinecraftClientMixin {
    ...
    public boolean klaxon$risingEdgeBlockAttackCheck(boolean original, @Local BlockHitResult hitResult) {
        if (interactionManager != null && player != null && HammerItem.canWallJump(player, player.method_48926().getBlockState(hitResult.getBlockPos()))) {

            // run walljump on client side
            HammerItem.processHammerWalljump(player, player.method_48926(), hitResult.getBlockPos(), hitResult.getSide());
